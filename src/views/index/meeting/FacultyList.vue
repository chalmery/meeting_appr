      ...
      <v-data-table
        ...
        <template v-slot:top>
          <v-toolbar flat prominent>
            ...
            <div class="mt-5 ml-5 text-h6">院系信息</div>
            <v-spacer></v-spacer>
            <!--搜索框-->
            <v-text-field v-model="pagination.queryString"  label="在此输入.." dense solo
                          hint="根据院系名称,地点筛选" color="success" class="mt-5"></v-text-field>
            <v-btn color="success" elevation="5" class="mt-5 ml-5" @click="search"><i class="fa fa-search"></i>搜索</v-btn>
            <v-btn color="deep-purple" class="white--text mt-5 ml-5" elevation="5" @click="clean"><i class="fa fa-times">重置</i></v-btn>
            <v-spacer></v-spacer>
            <!--新建院系按钮-->
            <v-btn color="success" elevation="5" dark class="mt-5" @click="showCreateDialog">新添系楼</v-btn>
          </v-toolbar>
        </template>
        <!--自定义会议状态列-->
        <template v-slot:item.teach="{ item }">
          <div v-if="item.teach===true">
            <v-chip class="elevation-5"  color="success" label>是</v-chip>
          </div>
          <div v-if="item.teach===false">
            <v-chip class="elevation-5" color="warning" label>否</v-chip>
          </div>
        </template>
    ...
    <v-form v-model="valid"  ref="form">
      <v-dialog v-model="dialog" max-width="400px" transition="dialog-top-transition">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container>
              <v-text-field color="success" v-model="editedItem.name" label="系楼名称" :rules="[rules.required,rules.length(20)]"></v-text-field>
              <v-text-field color="success" v-model="editedItem.location" label="系楼地点" :rules="[rules.required,rules.length(10)]"></v-text-field>
              <v-radio-group mandatory v-model="editedItem.teach" row>
                <v-radio color="success" label="教学楼" :value="true"></v-radio>
                <v-radio color="warning" label="非教学楼" :value="false"></v-radio>
              </v-radio-group>
            </v-container>
          </v-card-text>
          <!--新建页面的保存，取消按钮-->
...
export default {
  ...
  methods:{
    ...
    save(){
      ...
      if (validate === true) {
        ...
        if (this.editedIndex===-1) {
          this.$axios.post("/faculty/add",this.editedItem,{
            //加入token
            headers: {
        ...
        //如果是修改
          ...
          this.$axios.post("/faculty/edit",this.editedItem,{
            //加入token
            headers: {
              'Content-Type': 'application/json',
              'token': this.$store.state.token.tokenValue,
    ...
    // 删除页面的删除按钮
    deleteItemConfirm () {
      this.$axios.post("/faculty/delete",{id:this.editedItem.id},{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
    ...
    findPage(){
      ...
      if (param.queryString==='是'){
        param.queryString=true;
      }else if(param.queryString==='否'){
        param.queryString=false;
      }
      this.$axios.post("/faculty/findPage",param,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
  ...
  computed: {
    ...
    formTitle () {
      return this.editedIndex === -1 ? '新添系楼' : '编辑系楼'
    },
  },
