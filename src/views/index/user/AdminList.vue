<template>
  <v-app  style="background-color: #EEEEEE">
    ...
    <v-dialog v-model="dialogUnBan" max-width="250px"   transition="dialog-top-transition">
      <v-card>
        <v-card-title class="subtitle-1">你确定要解封这个管理员吗?</v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  data(){
    return{
      ...
      foreverRules: [
        ...
        value => {
          let pattern = /^[0-9]*$/
          return pattern.test(value)|| '只能是正整数'},
      ],
      rules: {
  ...
  methods: {
    ...
    showDialog(){
      ...
      this.$axios.post("/faculty/findFacultyNameForTeach",null,{
        ...
        if (res.data.code === 200) {
          ...
          // 查询角色信息
          this.$axios.post("/role/findAllAdminRole",null,{
            //加入token
            headers: {
              'Content-Type': 'application/json',
              'token': this.$store.state.token.tokenValue,
    ...
    // 删除页面的删除按钮
    deleteItemConfirm () {
      this.$axios.post("/admin/delete/"+this.editedItem.id,null,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
      ...
      // 如果是修改保存
        ...
        if (validate === true) {
          ...
          if (this.img==='' || this.img===null){
            this.$axios.post("/admin/edit", this.editedItem,{
              //加入token
              headers: {
                'Content-Type': 'application/json',
                'token': this.$store.state.token.tokenValue,
          ...
          // 否则就代表修改了图片
            ...
            // 如果符合规范，走上传图片的这一请求
              ...
              formData.append('role',this.editedItem.role)
              formData.append('faculty',this.editedItem.faculty)
              this.$axios.post("/admin/editAndUpload",formData,{
                //加入token
                headers: {
                  'Content-Type': 'application/x-www-form-urlencoded',
                  'token': this.$store.state.token.tokenValue,
    ...
    findPage(){
      ...
      this.$axios.post("/admin/findPage",param,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
    ...
    //账号下线
    offline(){
      this.$axios.put("/admin/offline/"+this.editedItem.id,null,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
    ...
    //封禁
    ban(){
      this.$axios.put("/admin/ban/"+this.editedItem.id+"/"+this.banTime+"/"+this.forever+"/"+this.deleteMeetingApply,null,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
    ...
    showBanStatusDialog(item){
      //查询封禁剩余时间
      this.$axios.get("/admin/banTime/"+item.username,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
    ...
    unBan(){
      this.$axios.put("/admin/unBan/"+this.editedItem.username,null,{
        //加入token
        headers: {
          'Content-Type': 'application/json',
          'token': this.$store.state.token.tokenValue,
