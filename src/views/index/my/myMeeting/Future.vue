      ...
      <v-data-table
        ...
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <div class="mt-5 ml-5 text-h6">将要参加</div>
          </v-toolbar>
          <v-container>
            <v-row class="mt-3">
              <v-col cols="5">
              ...
              <v-simple-table fixed-header>
                <template v-slot:default>
                  ...
                  <tr>
                    ...
                    <td> <v-chip class="elevation-5" label color="success" >{{selectMeeting.approvalStatus}}</v-chip></td>
...
export default {
  ...
  methods: {
    ...
    // 分页查询此用户的将要参加的
    findPage(){
      this.pagination.meeting.sponsor.username = this.$store.state.username;
      // 当前页，每页显示记录数，查询条件
      let param ={
      ...
      this.$axios.post("/meeting/findPageByFuture/"+this.$store.state.username,param,{
        //加入token
        headers: {
